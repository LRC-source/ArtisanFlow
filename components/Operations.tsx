...
export const MarginGuard = () => {
    ...
    return (
        ...
        <Card title="Margin Guard™ Protection" className="luxury-card shadow-2xl shadow-black/20 overflow-hidden relative rounded-[3rem] border-none bg-[#0A0A0A] backdrop-blur-3xl p-3.5 sm:p-6 lg:p-12 w-full h-full">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-10 lg:gap-16 items-start">
                <div className="space-y-10">
                    <div className={`p-4 sm:p-5 lg:p-6 rounded-[2rem] transition-all duration-700 border ${isWarning ? 'bg-amber-500/5 border-amber-500/20' : 'bg-white/5 border-white/10 hover:bg-white/10'}`}>
                        <label className="flex items-center gap-3 sm:gap-4 text-[11px] font-sans font-bold text-white sm:text-white/40 uppercase tracking-[0.3em] mb-6">
                            <DollarSign size={18} className={isWarning ? 'text-amber-500' : 'text-[#C5A059]'} strokeWidth={1.5} /> 
                            Material Cost
                        </label>
                        <div className="relative">
                            <span className="absolute left-4 top-1/2 -translate-y-1/2 text-white/30 font-serif text-sm sm:text-base md:text-3xl sm:text-5xl lg:text-7xl font-black">$</span>
                            <Input 
                                type="number" 
                                value={materialCost} 
                                onChange={(e) => setMaterialCost(Number(e.target.value))}
                                className={`font-serif text-sm sm:text-base md:text-3xl sm:text-5xl lg:text-7xl font-black bg-white/5 border-white/10 rounded-2xl pl-10 pr-6 py-5 shadow-sm transition-all ${isWarning ? 'border-amber-500/50 focus:border-amber-500 focus:ring-amber-500/10' : 'focus:border-[#6A2C91] focus:ring-[#6A2C91]/10'}`}
                            />
                        </div>
                        {isWarning && (
                            <motion.div 
                    ...
                    <div className="p-4 sm:p-8">
                        <label className="block text-[11px] font-sans font-bold text-white sm:text-white/40 uppercase tracking-[0.3em] mb-6">Total Batch Units</label>
                        <Input 
                            type="number" 
                            value={units} 
                            onChange={(e) => setUnits(Number(e.target.value))}
                            className="font-serif text-sm sm:text-base md:text-3xl sm:text-5xl lg:text-7xl font-black bg-white/5 border-white/10 rounded-2xl px-6 py-5 shadow-sm focus:border-[#6A2C91] focus:ring-[#6A2C91]/10 transition-all"
                            placeholder="e.g. 100"
                        />
                    </div>
                </div>

                <div className="bg-white/5 border border-white/10 rounded-[2.5rem] p-3.5 sm:p-6 lg:p-12 shadow-inner h-full flex flex-col justify-center relative overflow-hidden group">
                    ...
                    <div className="space-y-8 relative z-10">
                        <div className="flex flex-col sm:flex-col sm:flex-col sm:flex-row justify-between items-start sm:items-center border-b border-white/5 pb-6">
                            <span className="text-sm sm:text-base lg:text-xl text-white sm:text-slate-400 leading-relaxed font-sans font-light text-white sm:text-white/60">Unit Cost:</span>
                            <span className="text-xl sm:text-3xl lg:text-5xl font-bold sm:font-black font-serif tracking-tight text-white mb-4">${costPerUnit.toFixed(2)}</span>
                        </div>
                        <div className="flex flex-col sm:flex-col sm:flex-col sm:flex-row justify-between items-start sm:items-center border-b border-white/5 pb-6">
                            <span className="text-sm sm:text-base lg:text-xl text-white sm:text-slate-400 leading-relaxed font-sans font-light text-white sm:text-white/60">2.2x Target Price:</span>
                            <span className="font-serif text-sm sm:text-base md:text-3xl sm:text-5xl lg:text-7xl font-black text-emerald-400 tracking-tight">${recommendedPrice.toFixed(2)}</span>
                        </div>
                        <div className="pt-6">
                ...
                <div className="h-full flex flex-col">
                    <LockedNode isLocked={userTier === 'Free Audit'} requiredTier="Artisan Flow Basic" onUpgrade={() => {}}>
                      ...
                      <div className="sticky bottom-4 z-50 md:static p-4 md:p-0 bg-[#0A0A0A]/90 md:bg-transparent backdrop-blur-xl md:backdrop-blur-none border border-white/10 md:border-none rounded-3xl md:rounded-none shadow-2xl md:shadow-none mt-10 w-full">
                          <Button onClick={() => toast.info('Accessing live production nodes...')} className="w-full bg-white text-black hover:bg-white/90 py-3 px-6 rounded-full font-sans font-bold text-[11px] uppercase tracking-[0.3em] shadow-2xl shadow-black/10 transition-all group overflow-hidden relative">
...
export const Operations = () => {
  ...
  return (
    <motion.div 
      ...
      className="p-4 sm:p-8 lg:p-10 space-y-6 sm:space-y-10 lg:space-y-12 max-w-[1800px] mx-auto pb-12 sm:pb-20 lg:pb-32"
    >
      <div className="flex flex-col md:flex-col sm:flex-col sm:flex-row justify-between items-start md:items-end gap-3 sm:gap-4 sm:p-10">
        <div>
            <motion.div 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 sm:p-10">
      ...
      <motion.div 
        ...
        className="p-3.5 sm:p-6 lg:p-12 bg-emerald-500/5 border border-emerald-500/20 rounded-[3rem] flex items-center gap-3 sm:gap-6"
      >
          <div className="w-8 h-8 sm:w-12 sm:h-12 bg-emerald-500/20 rounded-2xl flex items-center justify-center text-emerald-400">
              <CheckCircle size={28} strokeWidth={1.2} />
          </div>
          <div>
              <h4 className="text-emerald-400 font-serif text-sm sm:text-base md:text-3xl sm:text-5xl lg:text-7xl font-black tracking-tight mb-2">Omnichannel Sync Secure</h4>
...
const HubNode = ({ icon: Icon, title, desc, color = "text-[#6A2C91]", bg = "bg-white/5", delay = 0 }: any) => (
  <motion.div 
    ...
    className={`luxury-card ${bg} p-5 sm:p-12 rounded-[3rem] hover:-translate-y-2 transition-all duration-700 cursor-pointer group flex flex-col h-full relative overflow-hidden shadow-2xl shadow-black/20`}
  >
