    ...
    <div className="p-4 sm:p-8 lg:p-10 space-y-6 sm:space-y-10 lg:space-y-12 max-w-7xl mx-auto pb-8 sm:pb-12 lg:pb-20">
      <Modal isOpen={showAdd} onClose={() => setShowAdd(false)} title="Initialize Integrity Inspection">
          <div className="space-y-6 p-4">
              <Select value={newCheck.productName} onChange={e => setNewCheck({...newCheck, productName: e.target.value})} className="w-auto mx-auto py-1 px-3 text-[10px] rounded-full bg-white/5 border-white/10 text-white font-sans text-sm">
              ...
              <Select value={newCheck.status} onChange={e => setNewCheck({...newCheck, status: e.target.value as any})} className="w-auto mx-auto py-1 px-3 text-[10px] rounded-full bg-white/5 border-white/10 text-white font-sans text-sm">
      ...
      {qualityChecks.length === 0 ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 sm:gap-6">
              {qualityChecks.map(check => (
                  <div key={check.id} className="luxury-card bg-black/40 backdrop-blur-xl border border-white/10 rounded-[2.5rem] p-3.5 sm:p-6 lg:p-12 hover:shadow-2xl hover:border-[#C5A059]/50 transition-all group relative overflow-hidden">
