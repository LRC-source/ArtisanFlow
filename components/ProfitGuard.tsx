        ...
        >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:p-8">
                ...
                <div className="lg:col-span-1 space-y-6">
                    <div className="luxury-card bg-white/5 border border-white/10 p-4 sm:p-8 shadow-sm rounded-3xl">
                        <h3 className="text-sm font-sans font-bold uppercase tracking-widest text-white/50 mb-8">Production Inputs</h3>
                        <div className="space-y-8">
                            <div>
                                <label className="block text-[10px] font-sans font-bold text-white/40 uppercase tracking-widest mb-3">Total Material Cost</label>
                                <div className="relative">
                                    <span className="absolute left-4 top-1/2 -translate-y-1/2 text-white/40 font-serif text-xl">$</span>
                                    <Input 
                                        type="number" 
                                        value={materialCost} 
                            ...
                            <div>
                                <label className="block text-[10px] font-sans font-bold text-white/40 uppercase tracking-widest mb-3">Batch Units</label>
                                <Input 
                                    type="number" 
                                    value={units} 
                            ...
                            <div>
                                <label className="block text-[10px] font-sans font-bold text-white/40 uppercase tracking-widest mb-3">Proposed Price</label>
                                <div className="relative">
                                    <span className="absolute left-4 top-1/2 -translate-y-1/2 text-white/40 font-serif text-xl">$</span>
                                    <Input 
                                        type="number" 
                    ...
                    <div className={`p-8 border border-white/10 shadow-sm rounded-3xl transition-colors duration-500 ${isWarning ? 'bg-amber-500/10' : 'bg-emerald-500/10'}`}>
                        <div className="flex items-center gap-4 mb-6">
                            ...
                            <h4 className={`font-serif text-xl ${isWarning ? 'text-amber-400' : 'text-emerald-400'}`}>
                ...
                <div className="lg:col-span-2 space-y-8">
                    ...
                    <div className="p-[1.5px] rounded-[3rem] bg-gradient-to-r from-[#06B6D4] via-[#A855F7] via-[#D946EF] to-[#C5A059] shadow-[0_0_20px_rgba(168,85,247,0.2)]">
                        <div className="p-4 sm:p-10 border-none shadow-sm bg-[#0A0A0A] rounded-[3rem] relative overflow-hidden">
                            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 sm:p-8 relative z-10">
                                <div>
                                    <h3 className="text-sm font-sans font-bold uppercase tracking-widest text-white/50 mb-2">Total Batch Profitability</h3>
                                    <p className="text-6xl font-serif text-white tracking-tighter">${totalProfit.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                                </div>
                            ...
                            <div className="mt-12 pt-8 border-t border-white/10 grid grid-cols-3 gap-4 sm:p-8">
                    ...
                    <div className="p-4 sm:p-8 bg-white/5 rounded-[2rem] border border-white/10">
                        <h6 className="text-[10px] font-sans font-bold text-white/50 uppercase tracking-[0.3em] mb-4">Strategic Recommendations</h6>
                    ...
                    <div className="flex gap-4">
                        <Button onClick={() => toast.success('Exporting financial report...')} className="flex-1 h-16 bg-gradient-to-r from-[#06B6D4] via-[#A855F7] to-[#C5A059] text-white rounded-[2rem] font-sans font-bold text-[11px] uppercase tracking-[0.3em] hover:opacity-90 transition-all shadow-2xl">
...
const AnalysisCard = ({ title, value, subtitle, icon: Icon, highlight }: any) => (
    <div className={`p-8 rounded-3xl border transition-all duration-500 hover:shadow-2xl ${highlight ? 'bg-gradient-to-br from-[#6A2C91]/20 to-transparent border-[#6A2C91]/30' : 'bg-white/5 border-white/10'}`}>
        <div className="flex justify-between items-start mb-6">
            <div className={`relative inline-flex items-center justify-center w-12 h-12 rounded-[1rem] bg-white/[0.05] border border-white/20 backdrop-blur-xl shadow-[0_0_15px_rgba(255,255,255,0.1)] z-10`}>
                <span className={`absolute inset-0 rounded-[1rem] bg-gradient-to-r from-white/10 to-white/5 opacity-40 blur-md`}></span>
                <Icon size={20} className={`${highlight ? 'text-[#C5A059]' : 'text-white/70'} relative z-10`} strokeWidth={1.5} />
            </div>
            <span className="text-[10px] font-sans font-bold uppercase tracking-[0.3em] text-white/40">{subtitle}</span>
        </div>
        <h4 className="text-sm font-sans font-bold text-white mb-1">{title}</h4>
        <p className={`text-4xl font-serif tracking-tighter ${highlight ? 'text-[#C5A059]' : 'text-white'}`}>{value}</p>
    </div>
);

const StatMini = ({ label, value }: any) => (
    <div>
        <p className="text-[10px] font-sans font-bold uppercase tracking-[0.3em] text-white/40 mb-1">{label}</p>
