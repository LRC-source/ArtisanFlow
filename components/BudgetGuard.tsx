...
export const BudgetGuard: React.FC = () => {
    ...
    return (
        <div className="space-y-8 animate-in fade-in pb-8 sm:pb-12 lg:pb-20">
                ...
                <div className="flex bg-stone-100 p-1 rounded-2xl border border-stone-200">
                    {(['daily', 'weekly', 'monthly', 'yearly'] as const).map((t) => (
                        <button
                            ...
                            className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${timeframe === t ? 'bg-white text-[#6A2C91] shadow-lg' : 'text-white sm:text-gray-400 hover:text-gray-600'}`}
                        >
                            {t}
                        </button>
                    ))}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 sm:gap-6">
                ...
                <div className="lg:col-span-1 space-y-8">
                    <Card title="Architect's Target" className="rounded-[2.5rem] border-stone-100 shadow-xl overflow-hidden relative">
                         <div className="absolute top-0 right-0 p-3.5 sm:p-6 lg:p-12 opacity-5 text-purple-600"><Target size={80} /></div>
                         <div className="space-y-6 mt-4 relative z-10">
                            <div className="space-y-1">
                                <label className="text-[10px] font-black text-white sm:text-gray-400 uppercase tracking-widest ml-1">Set Your {timeframe} Limit ($)</label>
                                <Input 
                                    type="number" 
                                    value={tempValue} 
                                    onChange={(e) => setTempValue(e.target.value)}
                                    className="text-sm sm:text-base md:text-3xl sm:text-5xl lg:text-7xl font-black font-black rounded-2xl py-3 px-6 py-4"
                                />
                            </div>
                            <Button 
                    ...
                    <div className="bg-stone-900 p-3.5 sm:p-6 lg:p-12 rounded-[2.5rem] text-white shadow-2xl relative overflow-hidden group">
                         ...
                         {isAnalyzing ? (
                             ...
                             <p className="text-sm sm:text-base text-stone-500 italic">Establish factory link for real-time suggestions.</p>
                         )}
                    </div>
                </div>
                ...
                <div className="lg:col-span-2 space-y-8">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-6">
                        <Card title="Lola’s Proposal" className="bg-purple-50 border-purple-100 shadow-lg relative overflow-hidden">
                            <div className="absolute top-0 right-0 p-4 sm:p-6 opacity-[0.03] text-purple-600"><Sparkles size={100} /></div>
                    ...
                    <Card title="Strategic Financial Ledger" className="border-stone-100">
                        <div className="space-y-4 mt-4">
                            {[
                                ...
                                <div key={i} className="flex flex-col sm:flex-col sm:flex-col sm:flex-row items-start sm:items-center justify-between p-4 hover:bg-stone-50 transition-colors border-b border-stone-50 last:border-0 group">
                                    <div className="flex items-center gap-3 sm:gap-4">
                                        <div className="w-6 h-6 sm:w-8 sm:h-8 lg:w-10 lg:h-10 bg-white border border-stone-100 rounded-xl flex items-center justify-center text-white sm:text-gray-400 group-hover:text-[#6A2C91] transition-colors shadow-sm">
                                            <BarChart3 size={18} />
                                        </div>
                                        <div>
